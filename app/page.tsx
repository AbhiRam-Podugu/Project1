...
export default function Home() {
  ...
  const [silentHours, setSilentHours] = useState<SilentHour[]>([]);
  ...
  // Fetch silent hours on user change
  useEffect(() => {
    if (!user) {
      setSilentHours([]);
      return;
  ...
  // Schedule silent hour & send email
  ...
  // Update user and hide auth forms after login/signup
  ...
  // If not logged in: show login/signup buttons & forms
  if (!user) {
    return (
